...
- **FR-002**: Starting an exam MUST select 75 questions from the certification's
  pool; if the pool has fewer than 75, it MUST use all available questions and
  surface the actual count.
- **FR-003**: During an exam, the learner MUST be able to navigate freely between
  ...
  ends.
- **FR-004**: The exam MUST show a countdown timer for the exam duration; the timer
  MUST be wall-clock based and persist across reload/leave so it resumes with the
...
- **FR-007**: On submission, the system MUST show a result with the overall score
  (number correct / total, and percent), a pass/fail indication against a threshold,
  and a per-question review (chosen answer vs correct answer + explanation).
...
> The following are reasonable defaults; the ones marked **(confirm in clarify)**
> are scope-significant and should be confirmed via `/speckit-clarify`.

- **Timer duration (confirm in clarify)**: 180 minutes base, with an optional +30
  minutes (ESL accommodation) the learner can enable when starting — interpreting
  the user's "180분 + 30분". (Alternatives: fixed 210, or fixed 180.)
- **Question selection (confirm in clarify)**: 75 questions chosen at random from the
  pool, re-randomized for each new exam attempt.
- **Pass threshold (confirm in clarify)**: 75% correct shown as pass (informational;
  the real exam uses scaled 100–1000 scoring we cannot replicate from raw answers).
